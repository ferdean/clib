...
double trimesh::PI = 3.14159265358979323846264338327950288419716939937510582;
...
unsigned int trimesh::_test_bit_masks[32];     // = 2^i, i = 0, ..., 31.
unsigned int trimesh::_clear_bit_masks[32];    // = ~(2^i), i = 0, ..., 31.
unsigned int trimesh::_extract_bits_masks[32]; // = 2^i - 1, i = 0, ..., 31.
...
unsigned char trimesh::_vo[3] = {1, 2, 0};
unsigned char trimesh::_vd[3] = {2, 0, 1};
unsigned char trimesh::_va[3] = {0, 1, 2};

unsigned char trimesh::_enext_tbl[3] = {1, 2, 0}; // = _vo 
unsigned char trimesh::_eprev_tbl[3] = {2, 0, 1}; // = _vd

void trimesh::initialize_lookup_tables()
{
  for (int i = 0; i < 32; i++) {
...
Triang* TriEdge::get_segment()
{
  if (is_segment()) {
    ...
    TriEdge N = e1->on_seg;
    while (N.tri != NULL) {
      //assert(N.tri->vrt[N.ver] == e1); // use check_mesh()
...
void TriEdge::print()
{
  printf("x%lx v(%d)", (unsigned long) tri, ver);
  ...
  if (tri->vrt[2] != NULL) {
    printf(" [%d,%d,%d] ", org()->idx, dest()->idx, apex()->idx);
    ...
    printf(" segment [%d,%d] ", tri->vrt[0]->idx, tri->vrt[1]->idx);
  }
  printf("\n");
}
...
{
  printf("Tri: x%lx,", (unsigned long) this);

  if ((vrt[0] == NULL) || (vrt[1] == NULL)) {
    printf(" unused.\n");
    return;
  } else if (is_deleted()) {
    printf(" (tri deleted)\n");
  ...
  // It is either a triangle or a segment.
  if (vrt[2] != NULL) {
    printf(" [%d,%d,%d]", vrt[0]->idx,vrt[1]->idx,vrt[2]->idx);
  } else {
    printf(" [%d,%d]", vrt[0]->idx,vrt[1]->idx);
  }
...
void Vertex::print()
{
  ...
  printf("\n");
  printf("  (%g,%g) h(%g) w(%g)\n", crd[0], crd[1], crd[2], wei);

  if (adj.tri != NULL) {
    printf("  adj: "); adj.print();
  } else {
    printf("  adj: NULL\n");
  }
  if (on_seg.tri != NULL) {
    // print the vertex-to-segment map.
    printf("  on_seg: ");
    TriEdge S = on_seg;
    while (S.tri != NULL) {
      S.print();
  ...
  if ((tr_segs != NULL) && (tr_segs->objects > 0)) {
    ...
    if (tr_steiners && (tr_steiners->objects > 0)) {
      for (int i = 0; i < tr_steiners->used_items; i++) {
        ...
        if (v->on_seg.tri != NULL) { // v->is_fixed()
          TriEdge sseg = v->on_seg;
          if (sseg.tri == NULL) {
...
void Triangulation::reset_options()
{
  ...
  tr_recnttri = NULL;
  tr_nonconvex = false;
  tr_free_domain = false;
  tr_free_voronoi = false;
...
#endif
  op_tol_rel_gap = 1.e-8;
  op_tol_minangle = 1.e-3 * PI / 180.0; // Radian
  op_tol_cosminangle = cos(op_tol_minangle);
  op_tol_cosmaxangle = cos(PI - op_tol_minangle);
  op_tol_voronoi_minangle = 0.01 * PI / 180.0; // Radian

  op_use_refine_list = 0; // -r
  op_elem_growth_ratio = 0.0; // -r#
  ct_elem_limit = 0;
  ...
  op_smooth_criterion = SMOOTH_LAPLACIAN; // =1
  op_smooth_iter = 3;
  op_ada_use_intpoints = 2;
  op_save_inter_meshes = 0;
  op_metric_scale = 1.0; // -m#
  op_minlen = 0.0;
  op_maxarea = 0.0;
  op_minangle = 20.0;
  ...
  io_point_array = 0;
  io_noindices = 0;     // -IN
  io_firstindex = 0;    // -I0 or -I1
  io_keep_unused = 0;   // -IJ
  io_no_out_triang = 0; // -IM
  io_outedges = 0;
  io_out_voronoi = 0;
  io_out_ucd = 0;
  io_dump_to_ucd = 0;
  io_dump_lift_map = 0;
  io_save_sol_to_vtk = 0; // -Is
  io_save_inria_mesh = 0; // -Im
  io_commandline[0] = '\0';
  io_infilename[0] = '\0';
  io_outfilename[0] = '\0';
  io_omtfilename[0] = '\0';
  ...
  ct_unused_vrts = ct_segments = ct_unused_segs = 0;
...
void Triangulation::clean()
{
  ...
  if (tr_free_domain) delete OMT_domain;
  if (tr_free_voronoi) delete Voronoi;
}

void Triangulation::quit(int code)
{
  clean();
  throw code;
}
