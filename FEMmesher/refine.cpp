  ...
  arraypool *fqueue, arraypool *encsegs, arraypool *enctris)
{
  ...
  if (encpt != NULL) {
    ...
    if (e1->is_acute()) {
      if (e2->is_acute()) {
        /* [2020-06-23] Do not split it this way.
        ...
        */
        // [2020-06-23] Split (e1,e2) by the projection of encpt.
        L3 = sqrt(L1*L1 - h*h);
        split = L3 / L;
        newvrt->crd[0] = e1->crd[0] + split * (e2->crd[0] - e1->crd[0]);
        newvrt->crd[1] = e1->crd[1] + split * (e2->crd[1] - e1->crd[1]);
      ...
      L3 = sqrt(L1*L1 - h*h); // Using Pythagorean theorem
      split = L3 / L;
      newvrt->crd[0] = e1->crd[0] + split * (e2->crd[0] - e1->crd[0]);
      newvrt->crd[1] = e1->crd[1] + split * (e2->crd[1] - e1->crd[1]);
  ...
  if (io_with_metric) { // -m
    newvrt->on_omt = e1->on_omt; // for searching in OMT_domain (may not be used).
    TriEdge G = S;
    ...
    for (int i = 0; i < encsegs->used_items; i++) {
      ...
      if (!op_no_bisect) { // no -Y option
        if (get_edge(pE->vrt[0], pE->vrt[1], E)) {
          ...
          // Stop if the desried number of triangles is reached.
          if ((ct_elem_limit > 0) &&
              (tr_tris->objects > ct_elem_limit)) break;
    ...
    for (int i = 0; i < enctris->used_items; i++) {
      ...
      if (get_triangle(qtri->vrt[0], qtri->vrt[1], qtri->vrt[2], E)) {
        ...
        if (loc == LOC_ENC_SEG) {
          ...
          int iloc = loc;
          if (io_with_metric) {
            TriEdge Eomt = E.tri->vrt[0]->on_omt;
            newvrt->on_omt = Eomt; // For locating the vertex.
            set_vertex_metric(newvrt, Eomt, iloc); // interpolate mesh size for the new vertex.
          }
          tt[0] = E;
          if (loc == LOC_IN_TRI) {
            int fflag = FLIP_13;
...
int Triangulation::delaunay_refinement()
{
  ...
  if (op_elem_growth_ratio > 0.0) { // -r#
    int ntri = tr_tris->objects;
    ct_elem_limit = ntri * (1.0 + op_elem_growth_ratio);
  }

  if (op_use_refine_list) { // -r
    ...
    int i;
    for (i = 0; i < ne; i++) {
      if (get_edge(edge_jump_list[i].vrt[0], edge_jump_list[i].vrt[1], E)) {
        // Stop if we have used the desried number of Steiner points.
        //if (steinerleft == 0) break;
        // Stop if the desried number of triangles is reached.
        if ((ct_elem_limit > 0) &&
        ...
        if (E.is_segment()) {
          split_enc_segment(E, NULL, smtrflag, fqueue, encsegs, enctris);
          if (encsegs->objects > 0) {
            repair_encsegments(encsegs, enctris);
          }
          if (enctris->objects > 0) {
          ...
          Vertex *newvrt = (Vertex *) tr_steiners->alloc();
          newvrt->init();
          newvrt->crd[0] = 0.5 * (E.org()->crd[0] + E.dest()->crd[0]);
          newvrt->crd[1] = 0.5 * (E.org()->crd[1] + E.dest()->crd[1]);
          if (op_db_verbose > 2) {
            printf("    Split edge [%d,%d] at (%g,%g)\n",
                   E.org()->idx, E.dest()->idx,
                   newvrt->crd[0], newvrt->crd[1]);
          }
          newvrt->idx = io_firstindex + (ct_in_vrts + tr_steiners->objects - 1);
          newvrt->typ = STEINERVERTEX;
          //int loc = locate_point(newvrt, E, 1); // encflag = 1
          ...
          if (loc == LOC_ENC_SEG) {
            ...
            pE->vrt[1] = E.dest();
            pE->vrt[2] = NULL;
            if (smtrflag) pE->set_infect(); // due to mesh size.
            newvrt->set_deleted();
            tr_steiners->dealloc(newvrt);
          } else if (loc != LOC_IN_OUTSIDE) {
            // Insert the vertex.
            ...
            if (io_with_metric) { // -m
              TriEdge Eomt;
              newvrt->on_omt = E.tri->vrt[0]->on_omt; // For locating the vertex.
              set_vertex_metric(newvrt, Eomt, iloc); // interpolate mesh size for the new vertex.
            }
            tt[0] = E;
            if (loc == LOC_IN_TRI) {
  ...
  } // if (op_use_refine_list)

  if (op_no_bisect == 0) { // no -Y.
    TriEdge E;
