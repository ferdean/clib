...
//
// The command line:
//   trimesh xy.ele -m uv.ele
//   trimesh xyz.smesh -m uv.ele scale
// where:
...
{
  if (argc < 4) {
    //printf("Usage: trimesh xy.ele uv.ele\n");
    printf("Usage: trimesh xyz.smesh uv.ele scale [max_min_ratio]\n");
    return 0;
  }
  ...
  if (!Tr_xy->parse_commands(myargc, myargv)) {
    ...
    //printf("Usage: trimesh xy.ele uv.ele\n");
    printf("Usage: trimesh xyz.smesh uv.ele\n");
    delete Tr_xy;
    return 0;
  }
...
//==============================================================================
// The command line:
//   trimesh xyz.smesh uv.ele uv_adapt.ele
// where:
//   xyz.smesh    is the source mesh (of a curved surface)
//   uv.ele       is the mapped mesh on parameter domain (planar)
...
{
  if (argc < 4) {
    printf("Usage: trimesh xyz.smesh uv.ele uv_adapt.ele\n");
    return 0;
  }
...
{
  if (argc < 2) {
    printf("Usage: trimesh [-options] filename[.node, .ele]\n");
    return 0;
  }
  ...
  if (!Tr->parse_commands(argc, argv)) {
    // No input or wrong parameters.
    printf("Usage: trimesh [-options] filename[.node, .poly, .ele, .edge]\n");
    delete Tr;
    return 0;
  }
  ...
  if (Tr->tr_tris == NULL) {
    if (Tr->incremental_delaunay()) {
      if (Tr->tr_segs != NULL) {
        if (Tr->recover_segments(true)) { // save_flag = true
          Tr->set_subdomains();
        } else {
          Tr->save_triangulation();
          Tr->save_edges();
  ...
  if (Tr->tr_tris != NULL) {
    ...
    if (Tr->io_out_ucd) { // -Iu
      Tr->save_to_ucd(0, Tr->io_with_metric);
    }
    ...
    if (Tr->io_save_sol_to_vtk) { // -Is
      if (Tr->io_with_sol) {
        Tr->save_sol_to_vtk(-1, true); // meshidx=-1 (ignored)
        Tr->save_edge_energy_jump();
        Tr->save_refine_edge_list(); // *.edgem
      }
    }
    if (Tr->io_save_inria_mesh) { // -Im
      Tr->save_inria_mesh();
    }
  }
...
{
  if (argc < 2) {
    printf("Usage: trimesh_adapt_mesh [-options] filename[.ele, .mesh]\n");
    return 0;
  }
...
{
  if (argc < 3) {
    printf("Usage: trimesh_interpolate_solutions src.mesh dest.mesh\n");
    return 0;
  }
...
{
  if (argc < 3) {
    printf("Usage: save_solution_to_paraview mesh_with_sol.mesh iter\n");
    return 0;
  }
  ...
  sprintf(Tr->io_outfilename, "%s_view", fname);

  Tr->io_inria_mesh = 1;
  if (!Tr->read_mesh()) { // if (!Tr->read_inria_mesh()) {
  ...
  if (!Tr->reconstruct_mesh(0)) {
    printf("Failed to recreate the mesh %s.\n", argv[1]);
    delete Tr;
    return 0;
  }
...
//==============================================================================
void trimesh_draw_voronoi_mass_centers(Triangulation* Tr, Triangulation* OMT,
                                      bool show_powe_cell, int center_size,
                                      void* Painter, double Cx, double Cy, double Sxy)
{
    //QPainter *painter = (QPainter *) Painter;
    //qDebug()<<"trimesh draw voronoi";
    Tr->OMT_domain = OMT;
    ...
    printf("cccccccccccc trimesh_draw_voronoi_mass_centers()\n");
    ...
    printf("aaaaaaaaaaa trimesh_draw_voronoi_mass_centers()\n");
    ...
    for (int i = 0; i < Tr->ct_in_vrts; i++) {
      ...
      Vertex *ptlist = NULL;
      int ptnum = 0;
      if (Tr->get_powercell(mesh_vertex, false, &ptlist, &ptnum)) {
        if (show_powe_cell) {
    ...
    printf("bbbbbbbbbbbbb trimesh_draw_voronoi_mass_centers()\n");
    ...
    for (int i = 0; i < Tr->tr_steiners->used_items; i++) {
      ...
      printf("%d ddddddddd trimesh_draw_voronoi_mass_centers()\n", mesh_vertex->idx);
...
int anisotropic_CVT(int argc, char* argv[])
{
  ...
  //if (Tr->op_quality || (Tr->op_metric > 0)) {
    ...
    trimesh_draw_voronoi_mass_centers(Tr, Tr, true, 2, NULL, 0, 0, 0);
    */
...
int main(int argc, char* argv[])
{
  ...
  //return save_solution_to_paraview(argc, argv);
  //return status;
}
